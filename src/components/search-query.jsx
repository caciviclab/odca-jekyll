  ...
  return (
    <div>
      ...
      { hit.name ? <div><strong>Candidate:</strong>
        <a href={'/candidate/' + hit.election_location + '/' +
          hit.election_date + '/' + hit.candidate_slug}> {hit.name}</a> </div> : ''
      }
      { hit.office_title ? <div><strong>Office:</strong>
        {hit.office_title}</div> : ''
      }
